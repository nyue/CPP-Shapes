...
void QuadtreeArea::addItem(QuadtreeItem* item)
{
  ...
  else
  {
    ...
    else
    {
      ...
      bool stored = false;
      for (std::list<QuadtreeArea*>::iterator areaIterator = areas->begin();
           areaIterator != areas->end(); ++areaIterator)
      {
...
{
  for (std::list<QuadtreeItem*>::iterator iterator = items->begin(); iterator != items->end();
       ++iterator)
  {
    if (*iterator == item)
    {
...
bool QuadtreeArea::removeItem(QuadtreeItem* item)
{
  ...
  // Rederict to areas;
  for (std::list<QuadtreeArea*>::iterator iterator = areas->begin(); iterator != areas->end();
       ++iterator)
  {
...
std::list<QuadtreeItem*>* QuadtreeArea::getPossibleNearItems()
{
  ...
  for (std::list<QuadtreeItem*>::iterator itemIterator = items->begin();
       itemIterator != items->end(); itemIterator++)
...
void QuadtreeArea::splitArea()
{
  if (isLeaf())
  {
    ...
    areas->push_front(
      new QuadtreeArea(this, Rectangle(o + Vector(newSize.x, 0), newSize))); // Rechts Oben
    areas->push_front(
      new QuadtreeArea(this, Rectangle(o + Vector(0, newSize.y), newSize))); // Links Unten
    areas->push_front(
      new QuadtreeArea(this, Rectangle(o + Vector(newSize.x, newSize.y), newSize))); // Rechts Unten
...
{
  std::string output = prefix + "Area: " + "Items: " + std::to_string(this->getItemsCount()) +
    " IsLeaf: " + (this->isLeaf() ? "True" : "False") + " - " + rectangle.toString() + "\n";
