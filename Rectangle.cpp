...
bool Rectangle::containsPoint(Vector vector)
{
  ...
  return (vector.x >= position.x && vector.y >= position.y && vector.x <= other_corner.x &&
    vector.y <= other_corner.y);
}
...
{
  Vector cir_rec_len =
    (this->getCenter() - circle->getPosition()).getNormalized() * circle->getSize();
...
{
  Vector rec_cir_len =
    (circle->getPosition() - this->getCenter()).getNormalized() * circle->getSize();
